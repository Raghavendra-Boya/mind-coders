...
export default function PodcastDashboard() {
  ...
  // Reuse programs API for Podcast view + fetch categories
  useEffect(() => {
  ...
  // Podcast dashboard: treat as podcast if ProgramType === "Podcast"
  // OR if the name itself contains the word "podcast" (for existing data
  // created before ProgramType was introduced)
  const podcastRows = allPrograms.filter((row) => {
  ...
  const filtered = podcastRows.filter((row) =>
  ...
  return (
    <>
      <div className="min-h-screen flex flex-col text-[#0b132b] bg-white">
        ...
        {/* Podcast Section (reusing Programs data) */}
